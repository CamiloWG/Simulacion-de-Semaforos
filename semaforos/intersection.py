# semaforos/intersection.py - Cruce corregido
...
class Intersection:
    def __init__(
        ...
        # Estas son las coordenadas donde deben detenerse los vehículos
        self.stop_line_A = 0.0  # Carril A se detiene en la línea del cruce
        self.stop_line_B = 0.0  # Carril B se detiene en la línea del cruce
    ...
    def _check_cross_blocking(self):
        ...
        blocked_A = self.lane_A.has_stopped_beyond_intersection_within(self.stop_line_A)
        blocked_B = self.lane_B.has_stopped_beyond_intersection_within(self.stop_line_B)
        ...
        if blocked_A and blocked_B and not self.both_red:
            ...
            self.both_red = False
            count_A = self.lane_A.count_approaching_within(self.d, self.stop_line_A)
            count_B = self.lane_B.count_approaching_within(self.d, self.stop_line_B)
    ...
    def _apply_traffic_flow_rules(self):
        """Aplica las reglas de flujo de tráfico."""
        count_A = self.lane_A.count_approaching_within(self.stop_line_A)
        count_B = self.lane_B.count_approaching_within(self.stop_line_B)
    ...
    def _should_change_light(self, count_A: int, count_B: int):
        """Determina si el semáforo debe cambiar y hacia dónde."""
        ...
        current_stop_line = (
            self.stop_line_A if current_green == "A" else self.stop_line_B
        )
        ...
        if current_lane.has_stopped_beyond_intersection_within(current_stop_line):
            should_change = True
            reason = f"Regla 5: Vehículo detenido después del cruce en {current_green}"

        # Regla 4: No hay vehículos en verde, pero sí en rojo
        green_count = count_A if current_green == "A" else count_B
        red_count = count_B if current_green == "A" else count_A

        if not should_change and green_count == 0 and red_count > 0:
            should_change = True
            reason = (
                f"Regla 4: Sin tráfico en {current_green}, pero {red_count} esperando"
            )
        ...
        close_vehicles = current_lane.count_within_r_to_cross(self.r, current_stop_line)
